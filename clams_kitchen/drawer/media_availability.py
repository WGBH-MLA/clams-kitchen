...
def extract_filename_ci_url(url:str, ci_id:str, opfx) -> str:
    ...
    if start_index == -1:
        print(opfx + "Warning: SonyCi URL does not include 'cpb-aacip'.")
        ...
        if start_index == -1:
            print(opfx + "Failure: Media filename not found in SonyCi URL.")
            print(opfx + "URL: <" + url + ">") 
            return None;
    ...
    if end_index == -1:
        end_index = url.find("&", start_index) 
    if end_index == -1:
        print(opfx + "Failure: Could not find the end of the filename in the URL.")
        print(opfx + "URL: <" + url + ">") 
        return None
    ...
    if filename.lower().find("fake_proxy") > -1:
        print(opfx + f"Failure: SonyCi returned the file `{filename}` for this item.")
        return None
...
# %%
def check_avail(guid:str, media_dir_path:str, opfx:str="") -> str:
    """
    ...
    if len(matches) > 1:
        print(opfx + "Warning: More than one media file matches guid. " +
...
def make_avail(guid:str, ci_id:str, media_dir_path:str, opfx:str="") -> str:
    """
    ...
    for attempt in range(retries+1):
        ...
        if ci_url == "null":
            if attempt == retries:
                ...
                print(opfx + "Warning: `ci_url.sh` returned no URL for " + ci_id)
                delay = 5
                print(opfx + "Pausing for", delay, "seconds...")
                time.sleep(delay)
                print(opfx + "Trying again.")
        else:
            break

    # Received the URL
    # Do a little checking; then go get the file
    ci_filename = extract_filename_ci_url(ci_url, ci_id, opfx)
    ci_filename_ext = ""
    
    if ci_filename is None:
        print(opfx + "Failure: No valid filename returned in SonyCi URL.")
        return None
    elif len(ci_filename) < 3:
        print(opfx + "Failure: Filename returned in SonyCi URL was less than 3 characters.")
        return None
    else:
    ...
    if ci_filename_ext not in ["mp3", "mp4", "mov"]:    
        ...
        print(opfx + "Failure: Media filename", ci_filename, "does not have valid extension.")
        return None
    ...
    print(opfx + "Will save", ci_filename, "to: " + filepath)
